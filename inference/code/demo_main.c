...
*                                        
* @param[in]         yuvSrc                                         YUV src frame
* @param[in]         yuvDst                                         YUV dst frame
* 
* @return           0 if successful, otherwise an error number returned
*/ 
static int demo_img_copy(OPDEVSDK_VIDEO_FRAME_ST *yuvSrc,OPDEVSDK_VIDEO_FRAME_ST *yuvDst)
{
    int ret;
...
static int demo_put_yuv_frame(OPDEVSDK_VIDEO_FRAME_INFO_ST *frm)
{
    ...
    int free_buf = (alg_buf.buf_r + DEMO_MAX_ALG_BUF_COUNT - alg_buf.buf_w - 1) % DEMO_MAX_ALG_BUF_COUNT;
...
static int demo_video_get_thread()
{
    ...
    while(thread_flg)
    {
        ...
        unsigned int start = (((unsigned int)tv.tv_sec) * 1000 + ((unsigned int)tv.tv_usec) / 1000);
...
static int demo_mscale_prep(int *pchan, int *pgrpId, void **pms_hdl, OPDEVSDK_VIN_CHN_ST *pvin_info)
{
    ...
    *pchan = chan;
    ...
    //!!!注意!!! vb缓存深度用于mscale缩放和智能算法处理，如某分辨率的vb全部被占用(智能算法get后不release)，会导致mscale缩放没有vb可用，从而影响后续缩放视频输出
...
*
* @param[in]         pyuv_buf                                         YUV frame buffer struct
...
static int demo_alg_buf_init(DEMO_ALG_BUF *pyuv_buf,int width,int height )
{
    ...
    for(i = 0; i < DEMO_MAX_ALG_BUF_COUNT; i++)
    {
        ...
        pyuv_buf->frm[i].yuvFrame.pVirAddr[1] = pyuv_buf->frm[i].yuvFrame.pVirAddr[0]+ width * height;
...
* @return           0 if successful, otherwise an error number returned
*/ 
static int demo_alg_proc(OPDEVSDK_VIDEO_FRAME_INFO_ST *pfrm, OPDEVSDK_POS_TARGET_ST *ptarget ,
    int num,OPDEVSDK_POS_ALERT_INFO_ST *palarm)
{
    int i = 0;
    int offset = num % 400;
...
/** 
* @brief            one algorithm processing thread with sending target list to WEB for displaying 
*
* @param[in] 		none
* 
* @return           0 if successful, otherwise an error number returned
...
static int demo_alg_proc_thread_2()
{
    ...
    while(thread_flg)
    {
        if (check_weight_flag == 1)
        {
            ...
            pack_target.timeType = OPDEVSDK_POS_TIME_TYPE_1K;
            pack_target.packId = 0;//预留
            pack_target.timeStamp = timeStamp / 1000;//img1_frame.timeStamp / 1000;//hikflow算法处理耗时过大，时间戳取最新时间戳
            pack_target.attribute = OPDEVSDK_POS_POLYGON_ATTRI_NORMAL;
            //pack_target.tgtList.tgtNum  = 8;
            pack_target.tgtList.pTgt = &target[0];
            //send data to WEB for displaying
...
static void demo_socket_proc()
{
    ...
    while(1)
	{
        ...
        sleep(0.5);  // 确保另一个进程有响应，检查check_weight_flag = 1时给出ck_weight_result
...
{    
    ...
    else if (0 == strcmp("YUV", argv[2]))
    {
        ...
        #else
        {        
            /*Here's an example: getting YUV frames from media driver and adding intelligent targets to display in HK WEB*/
            //demo_yuv_buf_init(&yuv_buf,input_width,input_height);
            demo_alg_buf_init(&alg_buf,alg_width,alg_height);

            /*getting YUV frames to algorithm buffer*/
            ret = pthread_create(&yuv_proc_tid, NULL, (void *)demo_video_get_thread, NULL);
            //线程优先级默认填80，栈大小可填0(当前内部限制最低2M),args为透传参数,不需要填0，后面不填透传参数
            if(ret != 0)
            {
                DEMOPRT("devsdk_pthrd_creat_pthread error ret = 0x%x\n",ret);
