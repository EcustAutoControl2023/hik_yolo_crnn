...
static int check_plat_config(CONFIG_DATA_T *config_data, void *model_buffer, int model_size, int *flag_bin_info)
{
    ...
    if (*flag_bin_info == 1) ///< if bin carry info
    {
        ...
        {
            DEMOPRT((char*)"WARNING: The platform information is different between CONFIG and BIN, please check.\n");
        }
        ...
        {
            DEMOPRT((char*)"WARNING: The 'core_proc_type' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
        ...
        {
            DEMOPRT((char*)"WARNING: The 'test_data_type' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
        ...
        {
            DEMOPRT((char*)"WARNING: The 'n' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
        ...
        {
            DEMOPRT((char*)"WARNING: The 'c' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
        ...
        {
            DEMOPRT((char*)"WARNING: The 'h' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
        ...
        {
            DEMOPRT((char*)"WARNING: The 'w' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
    ...
    else ///< if BIN don't carry info
    {
        ...
        {
            DEMOPRT((char*)"WARNING: %s doesn't carry information, will choose CONFIG param in default.\n", \
                config_data->hikflow_model_path);
        }
        else {                                      ///<  if BIN don't carry info, MEANWHILE CONFIG hasn't all info
            DEMOPRT((char*)"ERROR: %s doesn't carry information, meanwhile your CONFIG is incomplete. please check your \
            BIN or CONFIG.\n", config_data->hikflow_model_path);
            return -1;
        }
    }
...
static int demo_alg_createModel(void **model_handle, CONFIG_DATA_T *config_data,void **model_buffer)
{
    ...
    params_info.proc_type   = (OPDEVSDK_HKA_DL_PROCESS_TYPE_EN)config_data->core_proc_type;
...
{    
    ...
    {
        DEMOPRT((char*)"ERROR: CONFIG 'test_data_type' just support ‘1’(BGR) or ‘2’(YVU420). Please check.\n");
        goto err;
    }
...
static int parse_configure_info(char *config_file_path, CONFIG_DATA_T *config_data)
{
    ...
    if (1 != res)
    {
        DEMOPRT((char*)"Error: There is no 'hikflow_model_path' in your config, This param must have.\n");
        return -1;
    }
    ...
    if (1 != res)
    {
        DEMOPRT((char*)"Error: There is no 'test_image_list' in your config, This param must have.\n");
        return -1;
    }
    ...
    if (1 != res)
    {
        DEMOPRT((char*)"Error: There is no 'result_path' in your config, This param must have.\n");
        return -1;
    }
...
int demo_alg_proc_fromCamera(OPDEVSDK_VIDEO_FRAME_INFO_ST *pfrm,OPDEVSDK_POS_TARGET_LIST_INFO_ST *ptarget, int check_weight_limit)
{
    ...
    // DEMOPRT((char*)"if the model is not detection,or the out format is different,\n    please change it \n");
    // DEMOPRT((char*)"----------------------------------------------------------------\n");
    int detectnum ;
...
int demo_alg_proc_fromFile()
{
    ...
    for (j = 0; j < total_pic_num; j = j + g_config_data.n)
    {
        ...
        DEMOPRT((char*)"blob_num = %d,shape[0] = %d,shape[1] =%d,shape[2] =%d,shape[3] =%d\n",hkann_out.blob_num,
