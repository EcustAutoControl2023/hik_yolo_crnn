...
static int check_plat_config(CONFIG_DATA_T *config_data, void *model_buffer,
  ...
  if (*flag_bin_info == 1) ///< if bin carry info
  {
    ...
    {
      DEMOPRT("WARNING: The platform information is different between CONFIG "
              "and BIN, please check.\n");
    }
    ...
    if (plat_type_bin != config_data->core_proc_type &&
      ...
      DEMOPRT("WARNING: The 'core_proc_type' is different between CONFIG:%d "
              "and BIN:%d, will choose BIN param in default.\n",
    ...
    if (dformat_bin != config_data->test_data_type &&
      ...
      DEMOPRT("WARNING: The 'test_data_type' is different between CONFIG:%d "
              "and BIN:%d, will choose BIN param in default.\n",
    ...
    if (batch_bin != config_data->n &&
      ...
      DEMOPRT("WARNING: The 'n' is different between CONFIG:%d and BIN:%d, "
              "will choose BIN param in default.\n",
    ...
    if (channel_bin != config_data->c &&
      ...
      DEMOPRT("WARNING: The 'c' is different between CONFIG:%d and BIN:%d, "
              "will choose BIN param in default.\n",
    ...
    if (height_bin != config_data->h &&
      ...
      DEMOPRT("WARNING: The 'h' is different between CONFIG:%d and BIN:%d, "
              "will choose BIN param in default.\n",
    ...
    if (width_bin != config_data->w &&
      ...
      DEMOPRT("WARNING: The 'w' is different between CONFIG:%d and BIN:%d, "
              "will choose BIN param in default.\n",
  ...
  } else ///< if BIN don't carry info
  {
    ...
    {
      DEMOPRT("WARNING: %s doesn't carry information, will choose CONFIG param "
              "in default.\n",
              config_data->hikflow_model_path);
...
static int demo_alg_createHandle(CONFIG_DATA_T *config_data, void *model_handle,
  ...
  if (1 == config_data->test_data_type) {
    ...
    DEMOPRT("ERROR: CONFIG 'test_data_type' just support ‘1’(BGR) or "
...
static int parse_configure_info(char *config_file_path,
  ...
  if (1 != res) {
    DEMOPRT("Error: There is no 'hikflow_model_path' in your config, This "
            "param must have.\n");
    return -1;
  }
  ...
  if (1 != res) {
    DEMOPRT("Error: There is no 'test_image_list' in your config, This param "
            "must have.\n");
    return -1;
  }
  ...
  if (1 != res) {
    DEMOPRT("Error: There is no 'result_path' in your config, This param must "
            "have.\n");
    return -1;
  }
...
int demo_alg_proc_fromCamera(OPDEVSDK_VIDEO_FRAME_INFO_ST *pfrm,
  ...
  // class,score,x,y,width,height,batch_index\n"); DEMOPRT("if the model is not
  // detection,or the out format is different,\n    please change it \n");
  // DEMOPRT("----------------------------------------------------------------\n");
...
int demo_alg_proc_fromFile() {
  ...
  for (j = 0; j < total_pic_num; j = j + g_config_data.n) {
    ...
    DEMOPRT(
        "blob_num = %d,shape[0] = %d,shape[1] =%d,shape[2] =%d,shape[3] =%d\n",
        hkann_out.blob_num, hkann_out.output_blob[0].shape[0],
        hkann_out.output_blob[0].shape[1], hkann_out.output_blob[0].shape[2],
