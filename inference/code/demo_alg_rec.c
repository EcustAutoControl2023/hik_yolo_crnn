...
#define OPDEVSDK_HIKFLOW_SIZE_ALIGN(size, align)  (((size) + ((align) - 1)) & (~((align) - 1)))
#define OPDEVSDK_HIKFLOW_SIZE_ALIGN_16(size)      OPDEVSDK_HIKFLOW_SIZE_ALIGN(size, 16)
...
/** 
* @brief        check_plat_config                                // Verify platform information
* @param[in]   CONFIG_DATA_T        *config_data                 // Structure of configuration information
*              void                 *model_buffer                // Virtual memory address
*              int                   model_size                  // Model size
*              int                  *flag_bin_info               // The flag that the configuration information is recorded in the bin file
* @return      status
* @see         
*/ 
static int check_plat_config(CONFIG_DATA_T *config_data, void *model_buffer, int model_size, int *flag_bin_info)
{
    OPDEVSDK_HIKFLOW_CHECK_ERROR(NULL == config_data, "check config error: the config_data is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR(NULL == model_buffer, "check model_buffer error: the model_buffer is null.", -1);
    ...
    if (0 == strncmp(start_point, dev_info, strlen(dev_info)))
    {
        *flag_bin_info = 1;
        ...
        for (int i = 0; i < in_blob_num_bin; ++i)
        {
            ...
            DEMOPRT("has get bin batch: %d\n", batch_bin);

            ptr_real = ptr_real + sizeof(int);
            channel_bin = *(int*)ptr_real;
            DEMOPRT("has get bin channel: %d\n", channel_bin);

            ptr_real = ptr_real + sizeof(int);
            height_bin = *(int*)ptr_real;
            DEMOPRT("has get bin height: %d\n", height_bin);

            ptr_real = ptr_real + sizeof(int);
            width_bin = *(int*)ptr_real;
            DEMOPRT("has get bin width: %d\n", width_bin);
        }
    }
    ...
    if (*flag_bin_info == 1) ///< if bin carry info
    {
        ...
        {
            DEMOPRT("WARNING: The platform information is different between CONFIG and BIN, please check.\n");
        }

        ///< CONFIG has 'core_proc_type' AND diff BIN, then choose BIN param in default
        if (plat_type_bin != config_data->core_proc_type && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->core_proc_type)
        {
            DEMOPRT("WARNING: The 'core_proc_type' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                   config_data->core_proc_type, plat_type_bin);
            config_data->core_proc_type = plat_type_bin;
        } 
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->core_proc_type) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->core_proc_type = plat_type_bin;
        }

        if (dformat_bin != config_data->test_data_type && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->test_data_type)
        {
            DEMOPRT("WARNING: The 'test_data_type' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                    config_data->test_data_type, dformat_bin);
            config_data->test_data_type = dformat_bin;
        }
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->test_data_type) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->test_data_type = dformat_bin;
        }

        if (batch_bin != config_data->n && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->n)
        {
            DEMOPRT("WARNING: The 'n' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                    config_data->n, batch_bin);
            config_data->n = batch_bin;
        }
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->n) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->n = batch_bin;
        }

        if (channel_bin != config_data->c && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->c)
        {
            DEMOPRT("WARNING: The 'c' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                    config_data->c, channel_bin);
            config_data->c = channel_bin;
        }
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->c) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->c = channel_bin;
        }

        if (height_bin != config_data->h && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->h)
        {
            DEMOPRT("WARNING: The 'h' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                   config_data->h, height_bin);
            config_data->h = height_bin;
        }
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->h) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->h = height_bin;
        }

        if (width_bin != config_data->w && OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG != config_data->w)
        {
            DEMOPRT("WARNING: The 'w' is different between CONFIG:%d and BIN:%d, will choose BIN param in default.\n", \
                    config_data->w, width_bin);
            config_data->w = width_bin;
        }
        else if (OPDEVSDK_NOT_FIND_PARAM_IN_CONFIG == config_data->w) ///< CONFIG hasn't info, assign value as BIN
        {
            config_data->w = width_bin;
        }
    ...
    {
        if (1 == config_data->flag_config_all_param) ///< if BIN don't carry info, AND CONFIG has all info
        {
            DEMOPRT("WARNING: %s doesn't carry information, will choose CONFIG param in default.\n", \
                config_data->hikflow_model_path);
        }
        else {                                      ///<  if BIN don't carry info, MEANWHILE CONFIG hasn't all info
            DEMOPRT("ERROR: %s doesn't carry information, meanwhile your CONFIG is incomplete. please check your \
            BIN or CONFIG.\n", config_data->hikflow_model_path);
            return -1;
        }
    }
...
* @see
*/
static int demo_alg_createModel(void **model_handle, CONFIG_DATA_T *config_data,void **model_buffer)
{
    FILE   *model_file   = NULL;
    int    model_size    = 0;
    int    demo_ret      = 0;
    ...
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL != *model_handle), "demo_alg_createModel: model_handle is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == config_data), "demo_alg_createModel: config_data is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == config_data->hikflow_model_path), "demo_alg_createModel: config_data->hikflow_model_path is null.", -1);
    ...
    {
        DEMOPRT("read model bin success\n");
    }
    ...
    if (model_size <= 0)
    {
        DEMOPRT("%s model_size<=0\n", config_data->hikflow_model_path);
        goto err;
    }


    int buf_size = OPDEVSDK_HIKFLOW_SIZE_ALIGN(model_size, G5_PAGE_ALIGN);
	ret = opdevsdk_mem_allocCache((void *)&g_model_phy_base, 
                                  model_buffer,
                                  (const char *)"Model_cache",
                                   buf_size);
    if (0 != ret)
    {
        DEMOPRT("opdevsdk_memAllocCache failed ret=%d\n", ret);
        goto err;
    }
    ...
    if (check_plat_config(config_data, *model_buffer, model_size, &flag_bin_info) != 0)
    {
        DEMOPRT("check plat config failed\n");
        goto err;
    }
    ...
    params_info.proc_type   = config_data->core_proc_type;
    
    //register function of private layer
    params_info.custom_cb.get_model_memsize = Custom_Layer_GetModelMemsize;
    params_info.custom_cb.create_model = Custom_Layer_CreateModel;
    params_info.custom_cb.get_memsize = Custom_Layer_GetMemsize;
    params_info.custom_cb.create = Custom_Layer_Create;
    params_info.custom_cb.reshape = Custom_Layer_Reshape;
    params_info.custom_cb.forward = Custom_Layer_Forward;

    ///< get memory size
    ret = opdevsdk_hikflow_GetModelMemSize(&params_info, g_hikflow_mem_tab_model);
    if (0 != ret)
    {
        DEMOPRT("opdevsdk_hikflow_GetModelMemSize ret=0x%x\n", ret);
        goto err;
    }
    else
    {
        DEMOPRT("opdevsdk_hikflow_GetModelMemSize succeed\n");
    }

    // alloc model memory
    demo_ret = opdevsdk_hikflow_Device_Alloc_Memtab(g_hikflow_mem_tab_model, OPDEVSDK_HKA_MEM_TAB_NUM);

    if (0 != demo_ret)
    {
        DEMOPRT("alloc_mem_tab model ret=0x%x\n", demo_ret);
        goto err;
    }
    
    ///<creat model handle
    ret = opdevsdk_hikflow_CreateModel(&params_info, g_hikflow_mem_tab_model, model_handle);
    if (0 != ret)
    {
        DEMOPRT("opdevsdk_hikflow_CreateModel failed. ret=0x%x\n", ret);
        goto err;
    }
    else
    {
        ...
        {
            DEMOPRT("opdevsdk_hikflow_CreateModel succeed.\n");
        }
        else {
            DEMOPRT("opdevsdk_hikflow_CreateModel failed.\n");
            goto err;
        }
    }
...
err:
    ...
    if (NULL != *model_handle)
    {
        ...
        if (0 != ret)
        {
            DEMOPRT("HIK_ALG_ReleaseModel ret=0x%x\n", ret);
        }
    }
    
    opdevsdk_hikflow_Device_Free_Memtab(g_hikflow_mem_tab_model, OPDEVSDK_HKA_MEM_TAB_NUM);
...
* @see
*/
static int demo_alg_createHandle(CONFIG_DATA_T *config_data, 
                                 void *model_handle, 
                                 OPDEVSDK_HIKFLOW_PARAM_ST *param_info_net, 
                                 void **net_handle)
{    
    ...
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == model_handle), "demo_alg_createHandle model_handle is null", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == config_data), "demo_alg_createHandle config_data is null", -1);
    ...
    {
        DEMOPRT("ERROR: CONFIG 'test_data_type' just support ‘1’(BGR) or ‘2’(YVU420). Please check.\n");
        goto err;
    }

   	param_info_net->in_blob_param[0].src_blob.space   = OPDEVSDK_HKA_MEM_SPACE_CACHED;    ///< kt2, g5
	...
	param_info_net->in_blob_param[0].src_blob.stride[3] = config_data->w * sizeof(unsigned char);
    ...
    if (0 != ret)
    {
        DEMOPRT("opdevsdk_hikflow_GetMemSize ret=0x%x\n", ret);
        goto err;
    }
    else
    {
        DEMOPRT("opdevsdk_hikflow_GetMemSize succeed\n");
    }

    ///< allocate network memory
    demo_ret = opdevsdk_hikflow_Device_Alloc_Memtab(g_hikflow_mem_tab, OPDEVSDK_HKA_MEM_TAB_NUM);
    
    if (0 != demo_ret)
    {
        DEMOPRT("alloc_mem_tab net ret=0x%x\n", demo_ret);
        goto err;
    }
    ...
    if (0 != ret)
    {
        DEMOPRT("opdevsdk_hikflow_Create ret=0x%x\n", ret);
        goto err;
    }
    else
    {
        if(*net_handle != NULL)
        {
            DEMOPRT("HIK_ALG_Create succeed\n");  
        }   
    }
...
err:
    ...
    if (NULL != *net_handle)
    {
        ...
        if (0 != ret)
        {
            DEMOPRT("HIK_ALG_Release ret=0x%x\n", ret);
        }
    }
    
    opdevsdk_hikflow_Device_Free_Memtab(g_hikflow_mem_tab, OPDEVSDK_HKA_MEM_TAB_NUM);
                            ...
                            unsigned long long *total)
{
    ...
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == net_handle), "demo_alg_Process: net_handle is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == config_data), "demo_alg_Process: config_data is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == param_info_net), "demo_alg_Process: param_info_net is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == pfrm), "demo_alg_Process: pfrm is null.", -1);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((NULL == hkann_out), "demo_alg_Process: hkann_out is null.", -1);
    ...
    hkann_in.in_blob[0].src_blob.space  = param_info_net->in_blob_param[0].src_blob.space;
    hkann_in.in_blob[0].src_blob.format = OPDEVSDK_HKA_FORMAT_NCHW;
    ///< hkann_in.in_blob[0].src_blob.type   = param_info_net->in_blob_param[0].src_blob.type;
    hkann_in.in_blob[0].src_blob.type = OPDEVSDK_HKA_DATA_U08;

    hkann_in.in_blob[0].src_blob.dim       = param_info_net->in_blob_param[0].src_blob.dim;
	...
	hkann_in.in_blob[0].src_blob.stride[3] = config_data->w * sizeof(unsigned char);
    hkann_in.in_blob[0].src_blob.stride[2] = config_data->h;
    
    ///< fd process
    gettimeofday(&time_begin, NULL);
    ret = opdevsdk_hikflow_Process(net_handle, 0,&hkann_in, sizeof(hkann_in), hkann_out, sizeof(*hkann_out));
    gettimeofday(&time_end, NULL);
    one_time = 1000000 * (time_end.tv_sec - time_begin.tv_sec) + (time_end.tv_usec - time_begin.tv_usec);
    *total += one_time;
    if (0 != ret)
    {
        DEMOPRT("HIK_ALG_Process failed ret=0x%x\n", ret);
        return  -2;
    }
    else
    {
        DEMOPRT("opdevsdk_hikflow_Process ret=0x%x, cost %llu us\n", ret, one_time);
    }
    return OPDEVSDK_S_OK;
}
...
* @see
*/
static int parse_configure_info(char *config_file_path, CONFIG_DATA_T *config_data)
{
    int res = 0;
    ...
    {
        DEMOPRT("configure file %s not exit\n", config_file_path);
        return -1;
    }
    ...
    res = fscanf(configure_file, "data_mem_type=%d\n", &config_data->data_mem_type);
    if (1 != res)
    {
    ...
    if (1 != res)
    {
        DEMOPRT("Error: There is no 'hikflow_model_path' in your config, This param must have.\n");
        return -1;
    }
    ...
    if (1 != res)
    {
        DEMOPRT("Error: There is no 'test_image_list' in your config, This param must have.\n");
        return -1;
    }

    ///< parse output file path
    res = fscanf(configure_file, "result_path=%s\n", &config_data->result_path[0]);
    if (1 != res)
    {
        DEMOPRT("Error: There is no 'result_path' in your config, This param must have.\n");
        return -1;
    }
...
int demo_alg_init(char *config_file_path)
{
    ...
    {
        DEMOPRT("parse configure info failed\n");
        return -1;
    }

    ///<load model
    ret = demo_alg_createModel(&g_model_handle, &g_config_data,&g_model_buffer);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((OPDEVSDK_S_OK != ret), "demo_alg_init: demo_alg_createModel error", -1);

    ///<creat handle
    ret = demo_alg_createHandle(&g_config_data, g_model_handle, &g_param_info_net, &g_net_handle);
    OPDEVSDK_HIKFLOW_CHECK_ERROR((OPDEVSDK_S_OK != ret),  "demo_alg_init: demo_alg_createHandle error", -1);
    return OPDEVSDK_S_OK;
}
...
int demo_alg_proc_fromCamera(OPDEVSDK_VIDEO_FRAME_INFO_ST *pfrm,OPDEVSDK_POS_TARGET_LIST_INFO_ST *ptarget, int check_weight_limit)
{
    ...
    ret = demo_alg_Process(g_net_handle, &g_param_info_net, &hkann_out, &g_config_data, pfrm->yuvFrame.pVirAddr[0],1,&total);
    if(OPDEVSDK_S_OK != ret)
    {
        DEMOPRT("demo_alg_Process error= 0x%x \n", ret);
        goto err;
    }
    ...
    for(int n = 0; n < detectnum; n++)
    {
        ...
        if((int)class_type == 2 || (int)class_type == 5 || (int)class_type ==6 || (int)class_type ==7)
        {
            ...
            DEMOPRT("y_max     = %f\n", y_max);
            DEMOPRT("check_weight_limit     = %f\n", check_weight_limit);
            if(y_max > check_weight_limit)
            {
                check_flag = 0;
                continue;
                DEMOPRT("*********the car is outside the weight*******");                
            }
            else
            {
                check_flag = 1;
                continue;
                DEMOPRT("*********the car is on the weight*******");
            }  
            break;
        }
        else
        ...
        if(ptarget->tgtList.tgtNum < MAX_OUTPUT_BOX_NUM)
        {
            ...
            ptarget->tgtList.pTgt[n].region.point[0].x = box_info->bbox.x / g_config_data.w;
            ptarget->tgtList.pTgt[n].region.point[0].y =  box_info->bbox.y / g_config_data.h;
            ptarget->tgtList.pTgt[n].region.point[1].x = box_info->bbox.x / g_config_data.w + box_info->bbox.w / g_config_data.w;
            ptarget->tgtList.pTgt[n].region.point[1].y =  box_info->bbox.y / g_config_data.h;
            ptarget->tgtList.pTgt[n].region.point[2].x = box_info->bbox.x / g_config_data.w + box_info->bbox.w / g_config_data.w;
            ptarget->tgtList.pTgt[n].region.point[2].y =  box_info->bbox.y / g_config_data.h + box_info->bbox.h / g_config_data.h;
            ptarget->tgtList.pTgt[n].region.point[3].x = box_info->bbox.x / g_config_data.w;
            ptarget->tgtList.pTgt[n].region.point[3].y =  box_info->bbox.y / g_config_data.h + box_info->bbox.h / g_config_data.h;
            // DEMOPRT("n %d alg_w %d alg_h %d x %f y %f w %f h %f  target num %d\n", 
            //     n, g_config_data.w, g_config_data.h, box_info->bbox.x, box_info->bbox.y,  
            //     box_info->bbox.w, box_info->bbox.h, ptarget->tgtList.tgtNum);
        }
    }
...
err:
    ...
    if (NULL != g_net_handle)
    {
        ...
        if (0 != ret)
        {
            DEMOPRT("opdevsdk_hikflow_Release ret=0x%x\n", ret);
        }
    }
    ...
    if (NULL != g_model_handle)
    {
        ...
        if (0 != ret)
        {
            DEMOPRT("opdevsdk_hikflow_ReleaseModel ret=0x%x\n", ret);
        }
    }
...
int demo_alg_proc_fromFile()
{
    ...
    DEMOPRT("total_pic_num:%d\n", total_pic_num);


    ///< allocate input images memory
    if (g_param_info_net.in_blob_param[0].src_format == OPDEVSDK_HKA_BGR)  ///< bgr format memory size
    {
        data_size = g_config_data.n * g_config_data.w * g_config_data.c * g_config_data.h;
    }
    else  ///< nv21 or nv12 format memory size
    {
        data_size = g_config_data.n * g_config_data.w * (g_config_data.h + (g_config_data.h >> 1));
    }


	ret = opdevsdk_mem_allocCache((void *)&phy_base, 
								  (void **)&(test_img_data),
								  (const char *)"mmcache",
								   data_size);
	if (0 != ret)
    ...
    for (j = 0; j < total_pic_num; j = j + g_config_data.n)
    {
        ...
        for (k = 0; k < g_config_data.n; k++)
        {
            ...
            DEMOPRT("process pic : %s\n",file_name);
                
            pic_file = fopen(file_name, "rb");
            if (NULL == pic_file)
            {
                DEMOPRT("[%s]pic file can't find\n", file_name);
                goto err;
            }
            ...
            {
                data_size = (g_config_data.h + (g_config_data.h >> 1)) * g_config_data.w;
            }
        ...
        ret = demo_alg_Process(g_net_handle, &g_param_info_net, &hkann_out, &g_config_data, test_img_data,k,&total);
        if(ret != 0)
        {
            DEMOPRT("demo_alg_Process error= 0x%x \n", ret);
            goto err;
        ...
        ret = snprintf(res_file_name, sizeof(res_file_name), "picture_%d_out.txt", j);
        FILE *result_fp      = NULL;
        result_fp = fopen(res_file_name, "w");
        float score = 0;
        // DEMOPRT("----------------------------------------------------------------\n");
        // DEMOPRT("this is general output \n");
        // DEMOPRT("you can get result in %s\n",res_file_name);
        // DEMOPRT("----------------------------------------------------------------\n");
        int n = hkann_out.output_blob[0].shape[0] * 
                hkann_out.output_blob[0].shape[1] * 
                hkann_out.output_blob[0].shape[2] * 
                hkann_out.output_blob[0].shape[3];
        DEMOPRT("blob_num = %d,shape[0] = %d,shape[1] =%d,shape[2] =%d,shape[3] =%d\n",hkann_out.blob_num,
                                  hkann_out.output_blob[0].shape[0],
                                  hkann_out.output_blob[0].shape[1],
                                  hkann_out.output_blob[0].shape[2],
                                  hkann_out.output_blob[0].shape[3]);
    ...
    DEMOPRT("Process avg time:%llu us\n", total / total_pic_num);
...
int demo_alg_deinit()
{
    ...
    if (NULL != g_net_handle)
    {
        ...
        DEMOPRT("HIK_ALG_ReleaseHandle ret=0x%x\n", hikflow_ret);
    }
